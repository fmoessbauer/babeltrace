                                             ...
                                             const bt_stream *ir_stream)
{
    ...
    if (trace->lttng_index_path) {
        lttng_index_path = g_string_new(trace->lttng_index_path->str);
    }
                               ...
                               const bt_packet *packet)
{
    ...
    if (stream->lttng_index) {
        uint64_t bits = bt_ctfser_get_offset_in_current_packet_bits(&stream->ctfser);
        stream->lttng_index_state.pkg_offset_in_file += (bits + 7) / 8;
    }
